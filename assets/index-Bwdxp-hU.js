 ...
 */const Rl=5*60;na("authIdTokenMaxAge");Nl("Browser");console.warn("⚠️ Firebase未設定。.envファイルにAPIキーを設定してください。");function Zs(i){const e=new Date(i),n=e.getFullYear(),r=String(e.getMonth()+1).padStart(2,"0"),o=String(e.getDate()).padStart(2,"0");return`${n}-${r}-${o}`}function At(i){const e=new Date(i),n=["日","月","火","水","木","金","土"];return`${e.getMonth()+1}月${e.getDate()}日（${n[e.getDay()]}）`}function Bt(){return Zs(new Date)}function Ml(i){const e=Math.floor(i/60),n=i%60;return`${String(e).padStart(2,"0")}:${String(n).padStart(2,"0")}`}function te(i){if(!i)return 0;const[e,n]=i.split(":").map(Number);return e*60+n}function eo(){return Date.now().toString(36)+Math.random().toString(36).slice(2,9)}function to(i,e,n,r){const c=ei(n-i),l=ei(r-e),u=Math.sin(c/2)**2+Math.cos(ei(i))*Math.cos(ei(n))*Math.sin(l/2)**2;return 6371*2*Math.atan2(Math.sqrt(u),Math.sqrt(1-u))}function ei(i){return i*(Math.PI/180)}function se(i){const e=document.createElement("div");return e.textContent=i,e.innerHTML}function R(i,e="info",n=3e3){const r=document.getElementById("toast-container"),o={success:"check_circle",error:"error",warning:"warning",info:"info"},c=document.createElement("div");c.className=`toast ${e}`,c.innerHTML=`
  ...
  `),document.getElementById("sf-cancel").onclick=Ce,document.getElementById("sf-save").onclick=async()=>{const c=document.getElementById("sf-name").value.trim();if(!c){R("氏名を入力してください","warning");return}const l=document.getElementById("sf-save");l.disabled=!0,l.textContent="保存中...";const u=document.getElementById("sf-address").value.trim();let p=(i==null?void 0:i.lat)||Q.lat,_=(i==null?void 0:i.lng)||Q.lng;if(u)try{await Ct();const S=await ao(u);S?(p=S.lat,_=S.lng):R("住所から座標を取得できませんでした","warning")}catch(S){console.warn("ジオコーディング失敗:",S)}const E={name:c,gender:document.getElementById("sf-gender").value,address:u,workStart:document.getElementById("sf-work-start").value,workEnd:document.getElementById("sf-work-end").value,lat:p,lng:_,skills:{},color:(i==null?void 0:i.color)||_s[Lt.length%_s.length],isActive:!0};for(const[S]of Object.entries(fi)){const w=document.querySelectorAll(`input[name="skill-${S}"]:checked`);E.skills[S]=Array.from(w).map(k=>k.value)}try{e?(await Fl(i.id,E),R("職員情報を更新しました","success")):(await no(E),R("職員を登録しました","success")),Ce(),await Ln()}catch(S){R("保存に失敗しました: "+S.message,"error")}finally{l.disabled=!1,l.textContent=e?"更新":"登録"}}}document.addEventListener("click",async i=>{const e=i.target.closest("[data-edit-staff]");if(e){const r=Lt.find(o=>o.id===e.dataset.editStaff);r&&lo(r)}const n=i.target.closest("[data-delete-staff]");if(n){const r=Lt.find(o=>o.id===n.dataset.deleteStaff);if(r&&await Ti("削除確認",`${r.name} を削除しますか？`))try{await Vl(r.id),R(`${r.name} を削除しました`,"success"),await Ln()}catch{R("削除に失敗しました","error")}}});let pi=[],Is=[];async function xn(){const i=document.getElementById("page-container");try{const[e,n]=await Promise.all([_e().catch(()=>[]),so().catch(()=>[])]);pi=e,Is=n}catch(e){console.error("データの取得に失敗",e)}i.innerHTML=`
    <div class="page-header">
      <h1 class="page-title">
  ...
  `),document.getElementById("cf-cancel").onclick=Ce,document.getElementById("cf-save").onclick=async()=>{const u=document.getElementById("cf-name").value.trim();if(!u){R("氏名を入力してください","warning");return}const p=document.getElementById("cf-save");p.disabled=!0,p.textContent="保存中...";const _=document.getElementById("cf-address").value.trim();let E=(i==null?void 0:i.lat)||Q.lat,S=(i==null?void 0:i.lng)||Q.lng;if(_)try{await Ct();const k=await ao(_);k?(E=k.lat,S=k.lng,R(`座標を取得しました: ${k.lat.toFixed(4)}, ${k.lng.toFixed(4)}`,"success")):R("住所から座標を取得できませんでした。事業所付近の座標を使用します。","warning")}catch(k){console.warn("ジオコーディング失敗:",k),R("座標取得に失敗。事業所付近の座標を使用します。","warning")}const w={name:u,careLevel:document.getElementById("cf-care-level").value,address:_,requiredServices:Array.from(document.querySelectorAll('input[name="cf-service"]:checked')).map(k=>k.value),requiredSkills:Array.from(document.querySelectorAll('input[name="cf-skill"]:checked')).map(k=>k.value),genderPreference:document.getElementById("cf-gender-pref").value,visitDuration:parseInt(document.getElementById("cf-duration").value)||60,timeWindow:{start:document.getElementById("cf-time-start").value,end:document.getElementById("cf-time-end").value},notes:document.getElementById("cf-notes").value.trim(),lat:E,lng:S,isActive:!0};try{e?(await jl(i.id,w),R("利用者情報を更新しました","success")):(await ro(w),R("利用者を登録しました","success")),Ce(),await xn()}catch(k){R("保存に失敗しました: "+k.message,"error")}finally{p.disabled=!1,p.textContent=e?"更新":"登録"}}}document.addEventListener("click",async i=>{const e=i.target.closest("[data-edit-client]");if(e){const r=pi.find(o=>o.id===e.dataset.editClient);r&&uo(r)}const n=i.target.closest("[data-delete-client]");if(n){const r=pi.find(o=>o.id===n.dataset.deleteClient);if(r&&await Ti("削除確認",`${r.name} を削除しますか？`))try{await ql(r.id),R(`${r.name} を削除しました`,"success"),await xn()}catch{R("削除に失敗しました","error")}}});let Fe=Bt();async function ld(){const i=document.getElementById("page-container");i.innerHTML=`
    <div class="page-header">
      <h1 class="page-title">
